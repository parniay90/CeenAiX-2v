...
export default function PatientDashboard() {
  ...
  const { navigateToPaymentSettings, navigateToChangePassword, navigateToSettings, navigateToTerms, navigateToPrivacy, navigateToHome } = useNavigation();
    ...
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "#F0F4F8", minHeight: "100vh", display: "flex" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 8px", marginBottom: 32 }}>
          <img
            src="/ChatGPT_Image_Feb_27,_2026,_11_29_01_AM copy copy copy.png"
            alt="CeenAiX Logo"
            style={{ height: sidebarOpen ? 40 : 36, width: "auto", objectFit: "contain" }}
          />
        </div>
              ...
              {profileMenuOpen && (
                <div className="profile-menu">
                  ...
                  <div style={{ padding: "8px 0" }}>
                    ...
                    <div className="profile-menu-item danger" onClick={navigateToHome}>
                      <span style={{ fontSize: 16 }}>🚪</span>
              ...
              {PRESCRIPTIONS.map(rx => (
                <div key={rx.id} className="rx-card" style={{ cursor: "pointer" }} onClick={() => setShowPrescriptionsModal(true)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14.5, fontWeight: 700, color: "#1A1A2E" }}>{rx.name}</div>
                      <div style={{ fontSize: 12.5, color: "#64748B", marginTop: 2 }}>{rx.frequency} · {rx.duration}</div>
                      <div style={{ fontSize: 12, color: "#94A3B8", marginTop: 3 }}>{rx.doctor} · {rx.date}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <span className={`badge ${rx.status === "Active" ? "badge-green" : "badge-teal"}`}>{rx.status}</span>
                    <button className="btn-outline" style={{ fontSize: 12, padding: "6px 12px" }}>Share to Pharmacy</button>
                  </div>
                </div>
              ))}
            </div>
